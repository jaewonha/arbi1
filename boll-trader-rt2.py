    ...
    def __init__(self):
        ...
        if useCached:
            ...
            if backTesting and backTestingSrc == 'ob-dump':
                ...
                end_ts = str(time.mktime( fromTimeObj.timetuple() ))
                from_ts = str(time.mktime( (fromTimeObj - timedelta(minutes=3*20)).timetuple() ))
            else:
                ...
                from_ts = str(1640076245.0) #3
                end_ts = None
            print(f'from_ts:{from_ts}') 
            klines = ex.binance.get_historical_klines(config['symbol'], min, from_ts, end_ts)
